	...
	public void setFlightDelayLimit(Scenario scenario){
		...
		for(Flight f:scenario.flightList) {
			...
			if(f.initialTakeoffT > 1440*7 && f.initialTakeoffT <= 1440*8){
				...
				if(AirportID_49_50_61.contains(f.leg.originAirport.id)){
					...
					if(f.initialTakeoffT > 1440*7+17*60){
						...
						int periodEnd = f.initialTakeoffT + maximumDelayHrs*60;  //可以最多delay 8小时
						...
						int periodEnd = periodStart + maximumDelayHrs*60;  //起飞从17:00算起，可以最多delay 8 小时
				...
				//2. 如果起飞机场是间接受影响机场
					 ...
					 * 
					 */
					if(AirportID_49_50_61.contains(f.leg.destinationAirport.id) && f.initialLandingT <1440*7+17*60) {
						int periodStart = 1440*7+17*60 - (f.initialLandingT - f.initialTakeoffT);
						int periodEnd = periodStart + maximumDelayHrs*60;  //可以最多是delay 8小时
						int landingEnd = periodEnd +(f.initialLandingT - f.initialTakeoffT);  //用来检测是否进入机场关闭阶段
					...
					else if(formerAirportMap.get(f.leg.originAirport.id)!=null && AirportID_49_50_61.containsAll(formerAirportMap.get(f.leg.originAirport.id))
						...
						int periodEnd = periodStart + maximumDelayHrs*60;  //可以最多是delay 小时
						int landingEnd = periodEnd +(f.initialLandingT - f.initialTakeoffT);  //用来检测是否进入机场关闭阶段
						...
						int periodStart = f.initialTakeoffT;
						int periodEnd = f.initialTakeoffT + maximumDelayHrs*60;  //可以最多是delay 8小时
						int landingEnd = f.initialLandingT + maximumDelayHrs*60;  //用来检测是否进入机场关闭阶段
						//此处要判断最晚降落时间是否进入到达机场的机场关闭阶段
						if(AirportID_5_6_22_49_76.contains(f.leg.destinationAirport.id)){
							boolean vio = false;
							int airportCloseTime = 0;
				...
				//3. 降落机场属于49_50_61
					...
					if(f.initialLandingT > 1440*7+17*60){
						int periodStart = f.initialTakeoffT;
						int periodEnd = f.initialTakeoffT + maximumDelayHrs*60;  //可以最多是delay 8小时
						int landingEnd = f.initialLandingT + maximumDelayHrs*60;  //用来检测landing是否进入机场关闭阶段
						//此处要判断最晚降落时间是否进入到达机场的机场关闭阶段
						if(AirportID_5_6_22_49_76.contains(f.leg.destinationAirport.id)){
							boolean vio = false;
							int airportCloseTime = 0;
						...
						int periodEnd = periodStart + maximumDelayHrs*60;  //可以最多delay 8 小时
						int landingEnd = 1440*7+17*60 + maximumDelayHrs*60;  // delay 8 小时， landing time 从17:00开始后推
						//此处要判断最晚降落时间是否进入到达机场的机场关闭阶段
						if(AirportID_5_6_22_49_76.contains(f.leg.destinationAirport.id)){
							boolean vio = false;
							int airportCloseTime = 0;
				...
				//剩下的如果在7号早上10点之后，8号之前，则允许delay 小时
				else if(f.initialTakeoffT > 1440*7 +10*60 && f.initialTakeoffT <= 1440*8) {
					f.timeLimitList.add(new int[] {f.initialTakeoffT,f.initialTakeoffT+maximumDelayHrs*60});
				}
			...
			// 如果出发时间在7号之前的“国际航班”，降落时间在6号20:00之后，降落机场是49_50_61,也允许delay到[17:00,20:00]
				...
				int periodStart = 1440*7 + 17*60 - (f.initialLandingT - f.initialTakeoffT);
				int periodEnd = periodStart + 2*60;  //在17点的基础上，可以最多是delay 2小时
				f.timeLimitList.add(new int[] {periodStart,periodEnd});
			}
