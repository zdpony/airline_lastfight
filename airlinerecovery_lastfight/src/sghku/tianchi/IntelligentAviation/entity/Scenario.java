...
public class Scenario {
	...
	public Scenario(String filename) {
		...
		flightDelayLimitGenerator.setFlightDelayLimit(this);
	...
	public void readFlightSectionItinerary() {
		...
		for (Itinerary ite : itineraryList) {
			for (Flight f : ite.candidateFlightList) {
				int earlistT = f.initialTakeoffT+36*60;
				...
				for(int[] timeLimit:f.timeLimitList) {
					if(timeLimit[0] < earlistT) {
						earlistT = timeLimit[0];
					}
					if(timeLimit[1] > latestT) {
						latestT = timeLimit[1];
					}
				}
			...
			for (Flight f : ite.candidateFlightList) {
				for (FlightSection flightSection : f.flightSectionList) {
					...
					if (delay1 >= 0 && delay2 >= 0) {
						...
						for(int i=0;i<48;i++) {
							if(delay1 >= i*60 && delay2 <= (i+1)*60) {
								fsi.unitCost = getSignChangePassengerDelayParam(i+0.5);
								isFound = true;
								break;
							}
						}
						...
						if(!isFound) {
							if(delay2 <= 48*60){
								System.out.println("error 'not found!' "+delay1+" "+delay2+"  "+ite.flight.initialTakeoffT);
								for(int[] timeLimit:f.timeLimitList){
									System.out.print("["+timeLimit[0]+","+timeLimit[1]+"] ");
								}
								System.out.println();
								for(int i=0;i<48;i++) {
									System.out.print(i*60+","+(i+1)*60+"  ");
								}
								System.out.println();
