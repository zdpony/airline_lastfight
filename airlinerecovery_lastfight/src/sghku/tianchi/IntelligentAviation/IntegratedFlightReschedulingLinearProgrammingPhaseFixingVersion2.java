...
public class IntegratedFlightReschedulingLinearProgrammingPhaseFixingVersion2 {
	public static void main(String[] args) {
		...
		Parameter.gap = 5;
		Parameter.fixFile = "gap5_delay9_flightsection1h/fixschedule";
	...
	public static void runOneIteration(boolean isFractional, int fixNumber){
		...
		/*try {
			Scanner sn = new Scanner(new File("delayfiles/ana_linearsolutionwithpassenger_fulldelay_0901_stage1_589628.8_1549.csv"));
			sn.nextLine();
			while(sn.hasNextLine()){
				...
				for(String delay:delayStr){
					...
					if(!isInclude){
						System.out.println("error:"+f.id+"  "+delay+" "+f.takeoffTime+"->"+f.landingTime+"  "+f.leg.originAirport+"->"+f.leg.destinationAirport+" "+f.isDomestic+" "+f.initialTakeoffT+" actual:"+t);
						for(int[] timeLimit:f.timeLimitList) {
