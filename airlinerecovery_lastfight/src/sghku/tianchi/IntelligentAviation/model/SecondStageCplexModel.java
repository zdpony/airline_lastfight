		...
		try {
			...
			if (cplex.solve()) {
				...
				if (isFractional) {
					...
					int missConnection = 0;
					int firstFlightCancel = 0;
					int secondFlightCancel = 0;
					int bothFlightCancel = 0;
					
					for(TransferPassenger tp:sce.transferPassengerList) {
						if(!tp.inFlight.isCancelled && !tp.outFlight.isCancelled) {
							if(tp.inFlight.actualLandingT + tp.minTurnaroundTime > tp.outFlight.actualTakeoffT) {
								System.out.println("miss conn 1:"+tp.inFlight.actualLandingT+" "+tp.minTurnaroundTime+" "+tp.outFlight.actualTakeoffT+"  vol:"+tp.volume);
								missConnection += tp.volume;
							}		
						}
						if(!tp.inFlight.isCancelled && tp.outFlight.isCancelled) {
							System.out.println("miss conn 2:"+tp.inFlight.initialLandingT+" "+tp.minTurnaroundTime+" "+tp.volume+"  "+tp.inFlight.landingTime+" "+tp.inFlight.leg.originAirport+"->"+tp.inFlight.leg.destinationAirport+" "+tp.outFlight.id);
							missConnection += tp.volume;
						}
						if(tp.inFlight.isCancelled) {
							firstFlightCancel += tp.volume;
						}
						if(tp.outFlight.isCancelled) {
							secondFlightCancel += tp.volume;
						}
						if(tp.inFlight.isCancelled && tp.outFlight.isCancelled) {
							bothFlightCancel += tp.volume;
						}
					}
					System.out.println("missConnection:"+missConnection+"  firstConnCancel:"+firstFlightCancel+"  secondTransferCancel:"+secondFlightCancel+"  bothFlightCancel："+bothFlightCancel);
